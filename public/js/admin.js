...
async function loadStatistics() {
  try {
    ...
    async function drawCharts() {
      ...
      document.getElementById("siteRevenue").textContent =
        "€" + revenue.toFixed(2);
      ...
      new google.visualization.PieChart(
        document.getElementById("chart_bands_city")
      ).draw(bandsByCityData, optionsBandsCity);
      ...
      new google.visualization.BarChart(
        document.getElementById("chart_users_bands")
      ).draw(usersBandsData, optionsUsersBands);
      ...
      new google.visualization.PieChart(
        document.getElementById("chart_events")
      ).draw(eventsData, optionsEvents);
    }
  } catch (error) {
...
async function renderUserManagement() {
  try {
    ...
    tbody.innerHTML = info.usersResult
      .map(
        (user) => `
      <tr id="user-row-${user.username}">
        ...
        <td>${user.email}</td>
        <td>${user.city || "N/A"}</td>
        <td><button class="btn-small" onclick="DeleteUser('${
          user.username
        }')">Delete</button></td>
      </tr>
    `
...
async function RefreshReviews() {
  try {
    ...
    reviewsContainer.innerHTML = info.reviewsResult
      .map(
        (review) => `
